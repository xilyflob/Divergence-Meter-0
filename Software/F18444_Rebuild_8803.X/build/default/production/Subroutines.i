...
void DisplayBufSave(void)
{
    for (Pos=8; Pos--;)
    {
        DisplaySave[Pos] = DisplayBuf[Pos];
    }
}
...
void EEPROMBufferFill(uint8_t addr)
{
    for (Pos=8; Pos--;)
    {
        DisplayBuf[Pos] = EEPROM_Read(addr);
        addr--;
    }
}
...
void TubesOn(void)
{
    PWM6_LoadDutyValue(((5190 * Brightness) / 100));
...
void Pulse(void)
{
    PWM6_LoadDutyValue(519);
    _delay(104000);
    PWM6_LoadDutyValue(415);
    _delay(26000);
    PWM6_LoadDutyValue(363);
    _delay(21000);
    PWM6_LoadDutyValue(311);
    _delay(16000);
    PWM6_LoadDutyValue(207);
}
...
void TubeValuePicker (uint8_t start)
{
    ...
    for (Pos=start; Pos<8; Pos++)
    {
        ...
        while (1)
        {
            ...
            {
                DisplayBuf[Pos]++;
                if (DisplayBuf[Pos] == 11)
                    DisplayBuf[Pos] = 0;
            }
            else if (Temp == 2)
            {
                DisplayBuf[Pos]--;
                if (DisplayBuf[Pos] == 255)
                    DisplayBuf[Pos] = 10;
            }
            else
                break;
...
void RandomWL(void)
{
    ...
    else if (WLtype <= 15 && WLtype > 3)
        DisplayBuf[0] = 1;
    AnimBuf[0] = RunLength[RandomNum(7)];
    for (Pos=7; Pos>2; Pos--)
    {
        AnimBuf[Pos] = RunLength[RandomNum(63)];
    }
    while (1)
    {
        Action = 0;
    ...
    while (1)
    {
        ...
        for (Pos=8; Pos--;)
        {
            if (AnimBuf[Pos] != 0)
            {
                ...
                if (AnimBuf[Pos] == 0)
                {
                    DisplayBuf[Pos] = DisplaySave[Pos];
                    continue;
                }
                else
                {
...
{
    char regp;
    char shftregpln[12] = {0x4C,1,2,3,4,5,6,7,8,9,0,0x52};

    LATCbits.LATC6 = 0;
    for (Pos=0; Pos<8; Pos++)
    {
        for (regp=0; regp<12; regp++)
        {
            if (shftregpln[regp] == DisplayBuf[Pos])
            {
                LATCbits.LATC3 = 1;
                LATCbits.LATC5 = 1;
                LATCbits.LATC3 = 0;
...
uint8_t RandomNum(uint8_t upper)
{
    ...
    rand_num = (rand() % (upper + 1));
    return rand_num;
}

uint8_t Buttons(void)
{
    uint16_t presstime = 500;
    buttoncounter = 13;
...
uint8_t Bin2BCD(void)
{
    return (((BCDSplit[0] << 4) | BCDSplit[1]));
}

void BCD2Bin(uint8_t input)
{
    BCDSplit[1] = input & 0xF;
    input = (input >> 4) | (input << 4);
    BCDSplit[0] = input & 0xF;
}
